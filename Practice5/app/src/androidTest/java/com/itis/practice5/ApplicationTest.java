...
public class ApplicationTest {
    ...
    public void testFindCountryByCitiesCount() throws JSONException {
        ...
        for (int i = 0; i < expectedResults.length; i++) {
            List<Country> countries = CountriesHelper.findCountryByCitiesCount(getJSONObjects()[i]);
            assertEquals(expectedResults[i].length, countries.size());
            for (int j = 0; j < expectedResults[i].length; j++) {
                assertEquals(expectedResults[i][j], countries.get(j).getName());
            }
    ...
    public void testFindCityByPopulation() throws JSONException {
        ...
        for (int i = 0; i < expectedResults.length; i++) {
            List<City> cities = CountriesHelper.findCityByPopulation(getJSONObjects()[i]);
            assertEquals(expectedResults[i].length, cities.size());
            for (int j = 0; j < expectedResults[i].length; j++) {
                assertEquals(expectedResults[i][j], cities.get(j));
    ...
    public void testFindBiggestCountry() throws JSONException {
        ...
        for (int i = 0; i < expectedResults.length; i++) {
            ...
            else assertEquals(expectedResults[i], country.getName());
        }
    }
    ...
    public void testHasCountryWithCitiesWithLargeDistricts() throws JSONException {
        ...
        for (int i = 0; i < expectedResults.length; i++) {
            boolean result = CountriesHelper.hasCountryWithCitiesWithLargeDistricts(getJSONObjects()[i]);
